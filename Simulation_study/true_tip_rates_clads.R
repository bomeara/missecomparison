...
for(i in 1:length(tree_files)) {
  ...
  if(exists("tree")){
    ...
    label <- gsub("MalietEtAlRuns/MalietEtAl_ClaDS2_trees/","",label)
    n_tip <- Ntip(tree)
...
tree.dir <-  "../../original_trees/trees/ClaDS2/"
result.dir <- "../all_results/"
...
for(tree_index in 1:length(tree.files)){
  ...
  if(length(one_result)!=0) {
    ...
    event_data <- getEventData(tree_bamm, paste0(result.dir, one_result), nsamples = 500) 
...
############################################################
approaches <- c("BAMM","ClaDS", "MiSSEbest", "MiSSEavg")
