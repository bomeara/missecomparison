...
#try(phy <- ape::read.tree(paste0("data/title_rabosky_dryad/trees/", dir, "/", dir, ".tre")))
	...
	#print(phy)
	  #dir <- name(phy)
		...
		hisse_result_all <- hisse::MiSSEGreedy(phy, f=1, root.type=root_type, possible.combos=possibilities, chunk.size=12, n.cores=parallel::detectCores(), save.file=paste0(unname(Sys.info()["nodename"]), "_",tree_index, ".rda"), stop.deltaAICc=100)
