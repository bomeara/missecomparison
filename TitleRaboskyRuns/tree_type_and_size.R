...
for (i in seq_along(all_trees)) {
  ...
  tips_of_one_tree <- rates.cleaned[rates.cleaned$treeName==one_tree,]
...
organize_table <- function(results_all_trees, tree_type, parameter, error.measurement){
  ...
  results_one_tree_type_one_parameter$parameter <- factor(results_one_tree_type_one_parameter$parameter, levels=c("TB", "ND", "DR", "BAMM", "MiSSEbest", "MiSSEavg"))
