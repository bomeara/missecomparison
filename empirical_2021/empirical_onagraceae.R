...
# Fit MiSSE models
model.set = hisse::MiSSEGreedy(tree, f = 0.45, possible.combos = possible.combos, save.file=paste0(empirical.wd,"/Onagraceae_fit.Rsave"), 
...
for (model_index in 1:length(model.set)) {
  ...
  model.recons[[model_index]] <- hisse::MarginReconMiSSE(phy = model.set[[model_index]]$phy, f = 0.45, hidden.states = max(c(nturnover, neps)), 
...
save(model.set, model.recons, tip.rates, file="Onagraceae_example.Rsave")
